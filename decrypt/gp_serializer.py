...
Generate Json Data For Example(ExtJs 4.1) ::
    var data = {
            users: [
...
Usage ::
    ...
    result = serializers.serialize("extjs", Audit.objects.using('decrypt').all())
...
class Serializer(PythonSerializer):
    ...
    def start_serialization(self):
        ...
        self.total = self.json_kwargs.pop('total', 0)
        self.options.pop('total', 0)
        if self.options.get('indent'):
            # Prevent trailing spaces
            self.json_kwargs['separators'] = (',', ': ')
        self.stream.write("{")
        self.stream.write("\n")
        self.stream.write("'records': ")
        self.stream.write("[")
